...
// of two.
inline constexpr bool is_valid_range_mask(uintptr_t mask) {
  return (mask & (mask + 1)) == 0;
}
// Checks if an address is aligned with regard to a mask.
inline constexpr bool is_aligned_to_mask(uintptr_t address, uintptr_t mask) {
...
inline constexpr bool is_valid_range(uintptr_t base, uintptr_t mask) {
...
inline constexpr bool is_page_aligned(uintptr_t address) {
  return is_aligned_to_mask(address, page_size() - 1);
}

// The smallest number of pages needed to store an amount of memory.
inline constexpr size_t pages_needed_for(size_t memory_size) {
...
inline void set_bitmap_bit(phys_ptr<size_t> bitmap, size_t bit, bool value) {
  ...
  const size_t mask = size_t(1) << (bit % bits_in_size_t);
